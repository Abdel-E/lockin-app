import { useState, useEffect, useRef, useCallback } from 'react';
import { io } from 'socket.io-client';
import { storage } from '../utils/storage';

const SYNC_SERVER_URL = import.meta.env.VITE_SYNC_SERVER_URL || 'http://localhost:4000';
const HEARTBEAT_INTERVAL = 5000;
const GRACE_PERIOD = 10000;

export function useSession() {
  const [currentSession, setCurrentSession] = useState(null);
  const [isConnected, setIsConnected] = useState(false);
  const [error, setError] = useState(null);
  
  const socketRef = useRef(null);
  const heartbeatRef = useRef(null);
  const graceTimerRef = useRef(null);
  const userIdRef = useRef(storage.getUserId());
  const isActiveRef = useRef(true);

  // Initialize socket connection
  useEffect(() => {
    const socket = io(SYNC_SERVER_URL, {
      transports: ['websocket', 'polling'],
      reconnection: true,
      reconnectionAttempts: 5,
      reconnectionDelay: 1000
    });

    socketRef.current = socket;

    socket.on('connect', () => {
      console.log('✅ Connected to sync server');
      setIsConnected(true);
      setError(null);
      
      socket.emit('presence:join', {
        userId: userIdRef.current,
        device: 'web'
      });
    });

    socket.on('disconnect', () => {
      console.log('❌ Disconnected from sync server');
      setIsConnected(false);
    });

    socket.on('connect_error', (err) => {
      console.error('Connection error:', err);
      setError('Failed to connect to sync server');
    });

    socket.on('session:started', (session) => {
      console.log('🚀 Session started:', session);
      setCurrentSession(session);
    });

    socket.on('session:stopped', (session) => {
      console.log('⏹️ Session stopped:', session);
      setCurrentSession(null);
      
      // Save session to history
      const sessions = storage.getSessions();
      sessions.push(session);
      storage.saveSessions(sessions);
      
      // Update task hours
      updateTaskHours(session);
      
      // Show notification
      showNotification(session);
      
      // Stop heartbeat
      if (heartbeatRef.current) {
        clearInterval(heartbeatRef.current);
        heartbeatRef.current = null;
      }
    });

    socket.on('session:rejected', (data) => {
      console.log('❌ Session rejected:', data);
      setError(`Session already active on ${data.activeDevice}`);
      setTimeout(() => setError(null), 5000);
    });

    socket.on('sync:state', (data) => {
      if (data.session) {
        setCurrentSession(data.session);
      }
    });

    return () => {
      if (heartbeatRef.current) clearInterval(heartbeatRef.current);
      if (graceTimerRef.current) clearTimeout(graceTimerRef.current);
      socket.disconnect();
    };
  }, []);

  // Visibility change detection
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        isActiveRef.current = false;
        console.log('⚠️ Tab hidden - starting grace period');
        
        // Start grace period timer
        graceTimerRef.current = setTimeout(() => {
          if (!isActiveRef.current && currentSession) {
            console.log('⏱️ Grace period expired, ending session');
            stopSession('backgrounded');
          }
        }, GRACE_PERIOD);
      } else {
        isActiveRef.current = true;
        console.log('✅ Tab visible - canceling grace period');
        
        // Clear grace period
        if (graceTimerRef.current) {
          clearTimeout(graceTimerRef.current);
          graceTimerRef.current = null;
        }
      }
    };

    document.addEventListener('visibilitychange', handleVisibilityChange);
    
    return () => {
      document.removeEventListener('visibilitychange', handleVisibilityChange);
    };
  }, [currentSession]);

  // Heartbeat
  useEffect(() => {
    if (currentSession && socketRef.current) {
      heartbeatRef.current = setInterval(() => {
        socketRef.current.emit('session:heartbeat', {
          userId: userIdRef.current
        });
        console.log('💓 Heartbeat sent');
      }, HEARTBEAT_INTERVAL);

      return () => {
        if (heartbeatRef.current) {
          clearInterval(heartbeatRef.current);
        }
      };
    }
  }, [currentSession]);

  const startSession = useCallback((taskId) => {
    if (!socketRef.current || !isConnected) {
      setError('Not connected to sync server');
      return;
    }

    socketRef.current.emit('session:start', {
      userId: userIdRef.current,
      taskId,
      device: 'web'
    });
  }, [isConnected]);

  const stopSession = useCallback((reason = 'user') => {
    if (!socketRef.current) return;

    socketRef.current.emit('session:stop', {
      userId: userIdRef.current,
      reason
    });
  }, []);

  return {
    currentSession,
    isConnected,
    error,
    startSession,
    stopSession,
    userId: userIdRef.current
  };
}

// Helper functions
function updateTaskHours(session) {
  const tasks = storage.getTasks();
  const task = tasks.find(t => t.id === session.taskId);
  
  if (task) {
    const hoursCompleted = session.elapsedMins / 60;
    task.hoursRemaining = Math.max(0, task.hoursRemaining - hoursCompleted);
    storage.saveTasks(tasks);
    
    // Update daily stats
    const today = new Date().toISOString().split('T')[0];
    const dailyStats = storage.getDailyStats(today);
    dailyStats.hoursCompleted += hoursCompleted;
    dailyStats.sessionCount += 1;
    storage.saveDailyStats(today, dailyStats);
  }
}

function showNotification(session) {
  const tasks = storage.getTasks();
  const task = tasks.find(t => t.id === session.taskId);
  const reason = session.reason === 'backgrounded' ? 'Session ended - you left the app' : 
                 session.reason === 'timeout' ? 'Session ended - connection lost' :
                 'Session completed';
  
  console.log(`🎉 ${reason} - ${session.elapsedMins} minutes credited to ${task?.title || 'task'}`);
}
