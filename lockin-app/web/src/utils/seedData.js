...
export function initializeSeedData(storage) {
  // Only add seed data if there are no tasks
  const existingTasks = storage.getTasks();
  
  if (existingTasks.length === 0) {
    // Add a sample task
    storage.addTask('Complete Project Documentation', 5);
    storage.addTask('Study for Exam', 3);
    storage.addTask('Code Review', 2);
  }
}
